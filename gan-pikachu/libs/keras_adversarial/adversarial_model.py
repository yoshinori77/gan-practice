...
class AdversarialModel(Model):
    ...
    def adversarial_compile(self, adversarial_optimizer, player_optimizers, loss, player_compile_kwargs=None,
        ...
        if player_compile_kwargs is None:
            player_compile_kwargs = [{} for _ in self._layers]

        # Build player models
        for opt, model, compile_kwargs in zip(self.optimizers, self._layers, player_compile_kwargs):
        ...
        self.internal_input_shapes = filter_inputs(self._layers[0].input_shape)
        self.input_names = filter_inputs(self._layers[0].input_names)
        self.inputs = filter_inputs(self._layers[0].inputs)
        ...
        self.internal_output_shapes = collect(lambda m: m.output_shape)
    ...
    def constraints(self):
        if keras_2:
            ...
            return list(itertools.chain.from_iterable(model.constraints for model in self._layers))

    @property
    def updates(self):
        return merge_updates(list(itertools.chain.from_iterable(model.updates for model in self._layers)))

    @property
    def regularizers(self):
        return list(itertools.chain.from_iterable(model.regularizers for model in self._layers))
    ...
    def _make_train_function(self):
        ...
        if self.train_function is None:
            ...
            constraints = [{} for model in self._layers] if keras_2 else [model.constraints for model in self._layers]
