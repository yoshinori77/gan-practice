...
class RandomWeightedAverage(_Merge):
    ...
    def _merge_function(self, inputs):
        alpha = K.random_uniform((2, 1, 1, 1))
...
class WGANGP():
    ...
    def train(self, epochs, batch_size, sample_interval=50):
        ...
        gen = datagen.flow(X_train, batch_size=batch_size)  # 1枚しかないので、ミニバッチ数は1


        # Python ジェネレーターで9枚生成して、表示する。
        plt.figure(figsize=(10, 8))
        for batches in next(gen):
            gen_img = batches.astype(np.uint8)
        ...
        # self.critic_model.save(f'saved_model/critic_{date.today()}.h5')
        # self.generator_model.save(f'saved_model/generator_{date.today()}.h5')
...
if __name__ == '__main__':
    wgan = WGANGP()
    wgan.train(epochs=30000, batch_size=2, sample_interval=50)
