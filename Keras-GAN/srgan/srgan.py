...
class SRGAN():
    ...
    def load_data(self):
        ...
        for file in glob.glob(data_dirname):
            image = load_img(file).resize((self.lr_height, self.lr_width))
    ...
    def load_data2(self, epoch):
        data_dirname = os.path.join('dataset', '*')
        original_data = []
        for file in glob.glob(data_dirname):
            image = load_img(file).resize((self.hr_height, self.hr_width))
            array = img_to_array(image)
            original_data.append(array)
        data = np.array(original_data) / 255
        print(f'original_data shape: {data.shape}')
        index = epoch % data.shape[0]
        return data[index]
        # (xtrain, xtest, ytrain, ytest) = train_test_split(data, data, test_size=0.1)
        # print(f'xtrain shape: {xtrain.shape}, xtest shape: {xtest.shape}')
    ...
    def train(self, epochs, batch_size=1, sample_interval=50):
        ...
        for epoch in range(epochs):
            ...
            # _, imgs_lr = self.load_data()
            # imgs_hr = self.load_data2(epoch)
        ...
        self.discriminator.save(f'saved_model/discriminator_{date.today()}.h5')
        self.generator.save(f'saved_model/generator_{date.today()}.h5')
...
if __name__ == '__main__':
    gan = SRGAN()
    gan.train(epochs=30000, batch_size=2, sample_interval=50)
